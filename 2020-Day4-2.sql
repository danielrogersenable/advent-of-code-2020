...
WITH Passport_CTE AS (
	SELECT
		(SELECT COUNT(*) + 1 FROM #Input IC WHERE IC.RowNumber <= I.RowNumber AND IC.PassportLine = '') AS PassportNumber,
		PassportLine,
		CHARINDEX('byr:', I.PassportLine) AS ByrIndex,
		CHARINDEX('iyr:', I.PassportLine) AS IyrIndex,
		CHARINDEX('eyr:', I.PassportLine) AS EyrIndex,
		CHARINDEX('hgt:', I.PassportLine) AS HgtIndex,
		CHARINDEX('hcl:', I.PassportLine) AS HclIndex,
		CHARINDEX('ecl:', I.PassportLine) AS EclIndex,
		CHARINDEX('pid:', I.PassportLine) AS PidIndex,
		CHARINDEX('cid:', I.PassportLine) AS CidIndex
	FROM
		#Input I
),
Values_CTE AS (
	SELECT
		...
		CASE
			WHEN ByrIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, ByrIndex + 4, 4)
		END AS ByrValue,
		CASE
			WHEN IyrIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, IyrIndex + 4, 4)
		END AS IyrValue,
		CASE
			WHEN EyrIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, EyrIndex + 4, 4)
		END AS EyrValue,
		CASE
			WHEN HgtIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, HgtIndex + 4, 5)
		END AS HgtValue,
		CASE
			WHEN HclIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, HclIndex + 4, 7)
		END AS HclValue,
		CASE
			WHEN EclIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, EclIndex + 4, 3)
		END AS EclValue,
		CASE
			WHEN PidIndex = 0
			THEN ''
			ELSE SUBSTRING(PassportLine, PidIndex + 4, 10) -- TODO [DR] - this may need to be longer
		END AS PidValue
	FROM
		Passport_CTE
),
...
SELECT		PassportNumber,
			MAX(ByrValue) as ByrValue,
			MAX(IyrValue) as IyrValue,
			MAX(EyrValue) as EyrValue,
			MAX(HgtValue) as HgtValue,
			MAX(HclValue) as HclValue,
			MAX(EclValue) as EclValue,
			MAX(PidValue) as PidValue
FROM		Values_CTE
GROUP BY	PassportNumber
)
SELECT		COUNT(*)
FROM		Grouped_CTE
WHERE		(ISNUMERIC(ByrValue) = 1 AND CAST(ByrValue AS INT) >= 1920 AND CAST(ByrValue AS INT) <= 2002)
			AND
			(ISNUMERIC(IyrValue) = 1 AND CAST(IyrValue AS INT) >= 2010 AND CAST(IyrValue AS INT) <= 2020)
			AND
			(ISNUMERIC(EyrValue) = 1 AND CAST(EyrValue AS INT) >= 2020 AND CAST(EyrValue AS INT) <= 2030)
			AND
			(
				(HgtValue LIKE '[0-9][0-9][0-9]cm%' AND CAST(SUBSTRING(HgtValue, 0, 4) AS INT) >= 150  AND CAST(SUBSTRING(HgtValue, 0, 4) AS INT) <= 193)
				OR
				(HgtValue LIKE '[0-9][0-9]in%' AND CAST(SUBSTRING(HgtValue, 0, 3) AS INT) >= 59  AND CAST(SUBSTRING(HgtValue, 0, 3) AS INT) <= 76)
			)
			AND
			(HclValue LIKE '[#][a-f,0-9][a-f,0-9][a-f,0-9][a-f,0-9][a-f,0-9][a-f,0-9]')
			AND
			(EclValue IN ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'))
			AND
			(PidValue LIKE '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]%' AND PidValue NOT LIKE '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]%')