...
(
	SELECT
		SUBSTRING(Rules, 0, CHARINDEX(' bags', Rules)) as ParentBag,
		SUBSTRING(Rules, CHARINDEX(' bags', Rules) + 5, LEN(Rules)) as RemainingText
	FROM
		#Input
),
Recursive_CTE AS
(
	SELECT
		ParentBag,
		SUBSTRING(RemainingText, 0, 0) as ChildBag,
		RemainingText,
		PATINDEX('%[0-9]%', RemainingText) as NextNumberPosition,
		0 AS CurrentNumber,
		CHARINDEX(' bag', RemainingText) as NextBagText
	FROM
		Test_CTE
	UNION ALL
	SELECT
		ParentBag AS ParentBag,
		SUBSTRING(RemainingText, NextNumberPosition + 2, NextBagText - NextNumberPosition - 2) as ChildBag,
		SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText)) AS RemainingText,
		PATINDEX('%[0-9]%', SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText))) as NextNumberPosition,
		CAST(SUBSTRING(RemainingText, NextNumberPosition, 1) AS INT) AS CurrentNumber,
		CHARINDEX( ' bag', SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText))) as NextBagText
	FROM
		Recursive_CTE
	WHERE
		NextNumberPosition != 0 AND NextBagText - NextNumberPosition >= 0
),
Bags_CTE AS
(
	SELECT
		ParentBag,
		ChildBag,
		CurrentNumber AS NumberOfChildBags
	FROM
		Recursive_CTE
	WHERE
		ChildBag != ''
),
SecondRecursive_CTE AS
(
	SELECT
		ParentBag,
		ChildBag,
	...
	UNION ALL
		SELECT
			Par.ParentBag,
			Par.ChildBag,
			Par.NumberOfChildBags * Chi.NumberOfChildBags
		FROM
			SecondRecursive_CTE Chi
			INNER JOIN Bags_CTE Par on Par.ParentBag = Chi.ChildBag
)
SELECT
	SUM(NumberOfChildBags)
FROM
	SecondRecursive_CTE