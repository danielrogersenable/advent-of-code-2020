...
WITH GroupNumber_CTE AS
(
SELECT
	(SELECT COUNT(*) + 1 FROM #Input IC WHERE IC.InputRowId <= I.InputRowId AND IC.Answers = '') AS GroupNumber,
	Answers
FROM
	#Input I
),
-- Combine all answers for each group number into a single string
CombinedAnswers_CTE AS
(
	SELECT
		GroupNumber,
		STRING_AGG(Answers, '') AS CombinedAnswers,
		COUNT(*) - 1 AS NumberOfGroupMembers
	FROM
		GroupNumber_CTE
	GROUP BY
		GroupNumber
),
-- Recursively print the combined answer for each group number with the last character removed
Recursive_CTE AS 
(
SELECT
	GroupNumber,
...
FROM
	CombinedAnswers_CTE CAWL
	UNION ALL
	SELECT 
		GroupNumber,
		SUBSTRING(CombinedAnswers,0,LEN(CombinedAnswers)) AS CombinedAnswers,
		NumberOfGroupMembers
	FROM
		Recursive_CTE R
	WHERE
		LEN(CombinedAnswers) > 1
),
-- Determine for each instance of the reduced combined answer whether the last character occurs exactly n times
-- where n is the number of members in the group.
NewCharacterIdentified_CTE AS
(
	SELECT
		GroupNumber,
		CombinedAnswers,
		NumberOfGroupMembers,
		SUBSTRING(CombinedAnswers, LEN(CombinedAnswers), 1) AS LastCharacter,
		CASE
			WHEN LEN(REPLACE(CombinedAnswers, SUBSTRING(CombinedAnswers, LEN(CombinedAnswers), 1), '')) = LEN(CombinedAnswers) - NumberOfGroupMembers
			THEN 1
			ELSE 0
		END AS NewCharacterIdentified
	FROM
		Recursive_CTE
)
-- Sum the number of new characters identified
SELECT
	SUM(NewCharacterIdentified)
FROM
	NewCharacterIdentified_CTE
