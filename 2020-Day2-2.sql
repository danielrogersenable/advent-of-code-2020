...
INSERT INTO #Input (Range, Character, Password) VALUES 
...
WITH CTE (Range, LeftRange, RightRange, Character, Password) AS 
(
	SELECT		Range,
				CONVERT(int,LEFT(Range, CHARINDEX('-', Range) - 1)) as LeftRange,
				CONVERT(int, RIGHT(Range, LEN(Range) - CHARINDEX('-', Range))) as RightRange,
				Character,
				Password
	FROM		#Input
)
SELECT		COUNT(*)
FROM		CTE
WHERE		(SUBSTRING(Password, LeftRange, 1) = LEFT(Character, 1) AND SUBSTRING(Password, RightRange, 1) != LEFT(Character, 1))
			OR (SUBSTRING(Password, LeftRange, 1) != LEFT(Character, 1) AND SUBSTRING(Password, RightRange, 1) = LEFT(Character, 1))
