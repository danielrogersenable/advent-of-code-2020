...
WITH CTE (Range, LeftRange, RightRange, Character, Password, CharacterCount) AS 
(
	SELECT		Range,
				...
				Character,
				Password,
				LEN(Password) -LEN(REPLACE(Password, LEFT(Character,1), '')) as CharacterCount
	FROM		#Input
)
SELECT		COUNT(*)
FROM		CTE
WHERE		LeftRange <= CharacterCount
			AND RightRange >= CharacterCount