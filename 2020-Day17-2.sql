...
BEGIN
	...
	(
		...
		FROM		CubesToCheckAndIfActive_CTE CTC
					...
					(
						...
						AND ABS(CTC.W - IAS.W) <= 1
						AND (CTC.X != IAS.X OR CTC.Y != IAS.Y OR CTC.Z != IAS.Z OR CTC.W != IAS.W)
					)
		GROUP BY	CTC.X,
					CTC.Y,
					CTC.Z,
	...
	INSERT INTO		#InitialActiveStates (IterationCount, X, Y, Z, W)
