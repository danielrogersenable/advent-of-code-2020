...
BEGIN
	...
	WITH Surrounding_SeatTE AS (
		SELECT			RowIndex,
						ColumnIndex,
						SeatType AS CurrentSeatType,
						Iteration,
						(
							SELECT
								COUNT(*)
							FROM
								#IterationSeating Adj
							WHERE
								Adj.Iteration = @CurrentIteration
								AND ABS(Adj.RowIndex - Cur.RowIndex) <= 1
								AND ABS(Adj.ColumnIndex - Cur.ColumnIndex) <= 1
								AND ((Adj.RowIndex - Cur.RowIndex) != 0 OR (Adj.ColumnIndex - Cur.ColumnIndex) != 0)
								AND Adj.SeatType = '#'
						) as NumberOfSurroundingFullSeats
	...
	SELECT			RowIndex,
					ColumnIndex,
					CASE
						WHEN CurrentSeatType = 'L' AND NumberOfSurroundingFullSeats = 0
						THEN '#'
						WHEN CurrentSeatType = '#' AND NumberOfSurroundingFullSeats >= 4
						THEN 'L'
						ELSE CurrentSeatType
					END AS SeatType,
					@CurrentIteration + 1 AS Iteration,
					NumberOfSurroundingFullSeats
	FROM			Surrounding_SeatTE;
