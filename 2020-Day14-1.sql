...
MaskValue_CTE AS (
	SELECT		InputRowId,
				...
				NumberPower,
				CASE
					WHEN SUBSTRING(RowInstruction, @MaskLength - Number, 1) = '1'
					THEN NumberPower
					WHEN SUBSTRING(RowInstruction, @MaskLength - Number, 1) = '0'
					THEN 0
					ELSE NULL
				END AS MaskValue
	FROM		Mask_CTE
...
MemoryValue_CTE AS (
	SELECT		SUM(
					CASE
						WHEN MaskValue IS NOT NULL
						THEN MaskValue
						ELSE Mem.RowInstruction & NumberPower
					END
				) AS ValueToWrite,
				Mem.MemoryLocation,
				Mem.InputRowId
...
FROM		MemoryValue_CTE Mem
WHERE		InputRowId = (
				SELECT	MAX(InputRowId)
				FROM	MemoryValue_CTE MemJoin
