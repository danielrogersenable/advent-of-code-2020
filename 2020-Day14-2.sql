...
DECLARE @MaskLength int = 36;

WITH FormattedInstructions_CTE AS
(
	SELECT		InputRowId,
				SUBSTRING(Instruction, 0, CHARINDEX(' = ', Instruction)) AS RowType,
				SUBSTRING(Instruction, CHARINDEX(' = ', Instruction) + 3, 100) AS RowInstruction
	FROM		#Input
),
Numbers_CTE AS (
	SELECT	0 AS Number,
			CAST(1 AS BIGINT) AS NumberPower
	UNION ALL
	SELECT	Number + 1 AS Number,
			CAST(POWER(CAST(2 AS BIGINT), CAST(Number + 1 AS BIGINT)) AS BIGINT) AS NumberPower
	FROM	Numbers_CTE
	WHERE	Number < @MaskLength - 1
),
Mask_CTE AS
(
	SELECT		InputRowId,
				RowInstruction
	FROM		FormattedInstructions_CTE
	WHERE		RowType = 'mask'
),
Memory_CTE AS
(
	SELECT		InputRowId,
				CAST(SUBSTRING(RowType, 5, LEN(RowType) - 5) AS BIGINT) AS MemoryLocation,
				CAST(RowInstruction AS BIGINT) AS RowInstruction,
				(
					SELECT		MAX(InputRowId) 
					FROM		Mask_CTE M
					WHERE		M.InputRowId < FI.InputRowId
				) AS MaskId
	FROM		FormattedInstructions_CTE FI
	WHERE		RowType LIKE 'mem%'
),
MaskValue_CTE AS (
	SELECT		InputRowId,
				Number,
				NumberPower,
				SUBSTRING(RowInstruction, @MaskLength - Number, 1) AS MaskValueCharacter
	FROM		Mask_CTE M
	CROSS JOIN	Numbers_CTE
),
MaskValueAdded_CTE AS (
	SELECT		DISTINCT InputRowId AS InputRowId,
				CAST(0 AS BIGINT) AS AddedValue,
				-1 AS PreviousNumber
	FROM		MaskValue_CTE MV
	WHERE		MV.MaskValueCharacter = 'X'
	UNION ALL
	SELECT		MV.InputRowId,
				AddedValue + NumberPower AS AddedValue,
				MV.Number AS PreviousNumber
	FROM		MaskValue_CTE MV
	INNER JOIN	MaskValueAdded_CTE MVA ON MV.InputRowId = MVA.InputRowId
				AND MV.Number > MVA.PreviousNumber
	WHERE		MV.MaskValueCharacter = 'X'
),
MemoryValue_CTE AS (
	SELECT		SUM(
					CASE
						WHEN Mask.MaskValueCharacter = '1'
						THEN NumberPower
						WHEN Mask.MaskValueCharacter = '0'
						THEN Mem.MemoryLocation & Mask.NumberPower
						ELSE 0
					END
				) AS MinimumMemoryLocation,
				Mem.RowInstruction AS ValueToWrite,
				Mem.InputRowId,
				Mem.MaskId
	FROM		Memory_CTE Mem
	INNER JOIN	MaskValue_CTE Mask ON Mem.MaskId = Mask.InputRowId
	GROUP BY	Mem.InputRowId,
				Mem.RowInstruction,
				Mem.MaskId
),
FinalValues_CTE AS (
	SELECT		MV.InputRowId,
				MinimumMemoryLocation + MVA.AddedValue AS MemoryLocation,
				ValueToWrite
	FROM		MemoryValue_CTE MV
	INNER JOIN	MaskValueAdded_CTE MVA ON MV.MaskId = MVA.InputRowId
)
SELECT		* 
INTO		#Temp
FROM		FinalValues_CTE

SELECT		SUM(ValueToWrite)
FROM		#Temp Mem
WHERE		InputRowId = (
				SELECT	MAX(MemJoin.InputRowId)
				FROM	#Temp MemJoin
				WHERE	MemJoin.MemoryLocation = Mem.MemoryLocation)