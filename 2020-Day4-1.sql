...
SELECT		PassportNumber,
			MAX(ByrIndex) as HasByr,
			MAX(IyrIndex) as HasIyr,
			MAX(EyrIndex) as HasEyr,
			MAX(HgtIndex) as HasHgt,
			MAX(HclIndex) as HasHcl,
			MAX(EclIndex) as HasEcl,
			MAX(PidIndex) as HasPid,
			MAX(CidIndex) as HasCid
FROM		Passport_CTE
GROUP BY	PassportNumber
)
SELECT		COUNT(*)
FROM		Grouped_CTE
WHERE		HasByr * HasIyr * HasEyr * HasHgt * HasHcl * HasEcl * HasPid != 0