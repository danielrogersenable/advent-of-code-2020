...
(
	SELECT
		...
		RemainingText,
		PATINDEX('%[0-9]%', RemainingText) as NextNumber,
	...
	SELECT
		...
		SUBSTRING(RemainingText, NextNumber + 2, NextBagText - NextNumber - 2) as ChildBag,
		SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText)) AS RemainingText,
		PATINDEX('%[0-9]%', SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText))) as NextNumber,
		CHARINDEX( ' bag', SUBSTRING(RemainingText, CHARINDEX( ' bag', RemainingText) + 5, LEN(RemainingText))) as NextBagText
	FROM
		Recursive_CTE
	WHERE
		NextNumber != 0 AND NextBagText - NextNumber >= 0
),
Bags_CTE AS
(
	SELECT
...
(
	...
	UNION ALL
		...
		FROM
			Bags_CTE Par
			INNER JOIN SecondRecursive_CTE Chi ON Par.ChildBag = Chi.ParentBag
)
SELECT
	COUNT(DISTINCT ParentBag)
FROM
	SecondRecursive_CTE
WHERE
